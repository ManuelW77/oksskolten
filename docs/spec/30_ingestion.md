...
**Unreliable Content Detection (`hasReliableFullText`)**: Even after the RSS fallback, `full_text` can still end up shorter than `MIN_EXTRACTED_LENGTH` (e.g. feeds that only publish a short excerpt). `hasReliableFullText()` in `server/fetcher/content.ts` checks this (whitespace-collapsed length >= 200 chars) — but a length check alone isn't enough: an RSS teaser can coincidentally be long enough to pass the threshold while still not being the article's actual body (e.g. bot-protected sites where page extraction fails and the multi-sentence RSS description gets used as a fallback). The `full_text_is_excerpt` column on `articles` marks rows whose `full_text` came from the RSS-excerpt fallback (set in `fetchArticleContent` and `refreshStaleArticles` in `server/fetcher.ts`); `hasReliableFullText(fullText, isExcerptOnly)` treats such rows as unreliable regardless of length. This is used to:
